...
use _PA88H63MC84HH6TR4VD\Psr\Http\Message\MessageInterface;
...
class MessageFormatter
{
     ...
     * @return string
     */
    public function format(\_PA88H63MC84HH6TR4VD\Psr\Http\Message\RequestInterface $request, \_PA88H63MC84HH6TR4VD\Psr\Http\Message\ResponseInterface $response = null, \Exception $error = null)
    {
        $cache = [];
        return preg_replace_callback('/{\\s*([A-Za-z_\\-\\.0-9]+)\\s*}/', function (array $matches) use($request, $response, $error, &$cache) {
            ...
            switch ($matches[1]) {
                case 'request':
                    $result = \_PA88H63MC84HH6TR4VD\GuzzleHttp\Psr7\str($request);
                    break;
                case 'response':
                    $result = $response ? \_PA88H63MC84HH6TR4VD\GuzzleHttp\Psr7\str($response) : '';
                    break;
                case 'req_headers':
    ...
    private function headers(\_PA88H63MC84HH6TR4VD\Psr\Http\Message\MessageInterface $message)
    {
        $result = '';
