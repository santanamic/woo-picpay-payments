 ...
 */
class EachPromise implements \_PA88H63MC84HH6TR4VD\GuzzleHttp\Promise\PromisorInterface
{
    private $pending = [];
    /** @var \Iterator */
    private $iterable;
    ...
    {
        $this->mutex = false;
        $this->aggregate = new \_PA88H63MC84HH6TR4VD\GuzzleHttp\Promise\Promise(function () {
            reset($this->pending);
            ...
            while ($promise = current($this->pending)) {
                next($this->pending);
                $promise->wait();
                if ($this->aggregate->getState() !== \_PA88H63MC84HH6TR4VD\GuzzleHttp\Promise\PromiseInterface::PENDING) {
                    return;
                }
            }
        });
